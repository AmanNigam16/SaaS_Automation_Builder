...
**Standing reminder:** Re-read these rules before each phase, after context compaction, and before material changes; verify rather than guess, and keep this file concise and current as durable context.
...
- Verification: 14/14 focused tests pass, including prior OAuth/graph/Phase 3 regression coverage plus Gmail MIME safety and Gmail/Calendar graph validation. TypeScript `--noEmit`, `git diff --check`, and Next lint pass; lint reports only the documented pre-existing hook warnings. The optimized Next.js production build completes all 22 routes. Vercel Preview deployment `36RnJ3guzccfPx8byinF5oUSVMYP` for commit `d8b4a4d` reached READY. Authenticated browser verification used unsaved editor state to add Drive, Email, and Calendar nodes and confirmed all new action/configuration controls render with no framework error overlay. Save, Publish, Test configuration, Google reconnect, and all provider actions were deliberately not invoked, so no workflow, database, credit, or external-provider state changed.

### 2026-09-15 — Phase 4 Drive, webhook, and AI milestone

- Added a distinct Google Drive Action node so existing Google Drive triggers and stored workflows remain compatible. It supports upload, move, rename, copy, share, bounded download, and metadata; trigger configuration now supports optional file, folder, and removed/new-or-updated filtering. Drive data transfers are limited to 5 MB and require the full Drive scope at publish and execution time.
- Added an outbound Custom Webhook action with method, query, headers, body, expressions, 10-second timeout, capped response logging, HTTPS-only destinations, redirect rejection, credential-in-URL rejection, and private/local host checks. Persisted secret headers are deliberately not presented as safe; encrypted secret fields remain Phase 6.
- Added a Gemini AI action with generate/summarize/classify/extract modes, system instruction, expressions, structured JSON, bounded temperature/output, a 30-second timeout, usage metadata, sanitized errors, and a server-only `GEMINI_API_KEY`. Publishing is blocked until that key exists. No provider request was executed.
...
### 2026-09-15 — Phase 3 typed workflow semantics

- Added a versioned workflow-plan compiler over the existing React Flow JSON. It enforces one Drive trigger, connected acyclic graphs, supported executable nodes, explicit Condition true/false handles, filter behavior, and safe branching rules before save or publish. Legacy linear plans remain readable for existing workflows.
- Added typed node input/output schemas, preceding-step field tokens and `{{...}}` expressions, explicit Drive sample-data fetching, and local configuration tests. The editor now configures provider templates, AND/OR conditions, filters, formatters, fixed/until waits, and bounded loops using existing cards, accordions, controls, colors, and layout rather than a redesign.
- Extended the existing Prisma durable runner instead of adding a second workflow framework. Semantic steps persist inputs, outputs, sanitized failures, and execution state; Conditions route true/false paths, filters halt unmatched paths, formatters transform values, loops fan out with a 100-item ceiling, and waits persist their remaining queue for later continuation. Provider actions still use the existing Discord, Slack, and Notion paths and reserve credits only for action steps.
- Added a per-run wait-resume endpoint authenticated by a random one-time token whose hash alone is stored. Public HTTPS Production may schedule an external resume only when the existing server-side cron key is configured; protected Preview and localhost waits remain durable and owner-resumable after their due time. Scheduler failures now end visibly as failed runs rather than leaving them active.
- Removed the automatic Drive-file request from editor rendering; sample data is fetched only on request, eliminating that request waterfall. No database schema, Neon branch, OAuth provider configuration, Vercel setting, production deployment, or `main` branch was changed in this implementation.
- Verification before branch delivery: 10/10 focused Node tests pass (OAuth state, graph boundaries, expressions, conditions, formatter types, and waits); TypeScript `--noEmit` passes; `next lint` passes with only the recorded pre-existing hook warnings; `git diff --check` passes; and the optimized Next.js production build completes all 22 routes including `/api/cron/workflow-resume`. A local unauthenticated browser request safely redirects to Clerk sign-in with no Next.js error overlay. Authenticated Preview/editor runtime verification remains required before marking Phase 3 complete.
- The first authenticated Preview editor check caught a React maximum-update-depth failure in the new bidirectional node synchronization. Replaced the duplicated local/provider node state with the existing editor provider as the single node source of truth; React Flow changes now dispatch into that store directly. The correction again passes 10/10 tests, TypeScript, lint with only pre-existing warnings, whitespace checks, and the full production build. Push and recheck the replacement Preview before marking the phase complete.
- Corrected Preview `dpl_CLdVRAGYySKwYwHGaP9joWrhB3nf` reached READY. Authenticated browser verification loaded the editor with no active error overlay, added a Drive trigger and Condition in unsaved browser state, confirmed Wait/Formatter/Loop remain visible, and opened Condition configuration with branch/filter mode, AND/OR matching, field/operator controls, sample-data fetching, and configuration testing. Save and Publish were deliberately not used, so no workflow, provider, credit, or database state changed. Phase 3 is complete; deeper provider-specific execution belongs to Phase 4 and later integration verification.
